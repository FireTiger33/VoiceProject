...
import com.stacktivity.voiceproject.ui.fragments.*
...
class CallActivity : BaseActivity(), IncomeCallFragmentCallbackListener, QBRTCSessionStateCallback<QBRTCSession>,
    ...
    private fun startLoadAbsentUsers() {  // TODO improved algorithm
        // TODO check
        /*val usersFromDb = DbManager.getAllUsers()
    ...
    override fun onReceiveHangUpFromUser(session: QBRTCSession?, userId: Int?, map: MutableMap<String, String>?) {
        if (callService.isCurrentSession(session)) {
            if (userId == session?.callerID) {
                ...
                val wakeLock: PowerManager.WakeLock = pm.newWakeLock((PowerManager.SCREEN_BRIGHT_WAKE_LOCK | PowerManager.FULL_WAKE_LOCK | PowerManager.ACQUIRE_CAUSES_WAKEUP), «TAG»);
                wakeLock.acquire()*/
                window.addFlags(
...
//                    WindowManager.LayoutParams.FLAG_ALLOW_LOCK_WHILE_SCREEN_ON
            ...
            /*val participant = DbManager.getQbUserById(userId)
    ...
    private inner class CallServiceConnection : ServiceConnection {
        override fun onServiceDisconnected(name: ComponentName?) {
            Log.e("CallServiceConnection", "disconnected $name")
        }
        ...
        private fun login() {
            ...
            LoginService.start(this@CallActivity, checkNotNull(qbUser), pendingIntent)
        }
    }
